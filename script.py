...
TEST_PATH = '../input/data-science-bowl-2018/stage2_test_final/'
#'../input/data-science-bowl-2018/stage1_test/'
...
#np.int((l*9)/10)
print(v)
train_train_ids=train_ids[:v]
train_val_ids=train_ids[v:]
...
class NucleiDataset(Dataset):
    ...
    def load_nuclei(self,train_ids):
        ...
        for n, id_ in tqdm(enumerate(train_ids), total=len(train_ids)):
            path = TRAIN_PATH + id_
            image_path=path + '/images/' + id_ + '.png'
    ...
    def load_mask(self, image_id):
        ...
        #mask = np.zeros((info['height'], info['width'], 1), dtype=np.bool)
        i=0
        ...
        for mask_file in next(os.walk(mask_path))[2]:
            ...
            #resize(mask_, (IMG_HEIGHT, IMG_WIDTH), mode='constant', 
            #                              preserve_range=True)
                                    , axis=-1)
            mask[:, :, i:i+1] = mask_
            #print (mask.shape)
            ...
            #print(len(masks))
        #mask=np.array(masks)
        masks=np.zeros([info['height'], info['width'], i], dtype=np.uint8)
        masks=mask
...
COCO_MODEL_PATH = os.path.join(ROOT_DIR, 'mask_rcnn_coco (2).h5')
IMG_CHANNELS=3
"""
# Training dataset
dataset_train = NucleiDataset()
dataset_train.load_nuclei(train_train_ids)

dataset_train.prepare()

# Validation dataset
dataset_val = NucleiDataset()
dataset_val.load_nuclei(train_val_ids)
...
if init_with == "imagenet":
    ...
    model_path = '../input/mask-rcnn-coco-1/mask_rcnn_nuclei_1.h5'
...
#model.keras_model.save_weights("mask_rcnn_nuclei_1_30.h5")
"""            
...
print(os.listdir("../"))
print(os.listdir("../working"))
ldir=os.listdir("../working")
key ='nuclei'
ldir = filter(lambda f: f.startswith(key), ldir)
dir_names = sorted(ldir)
#print(dir_names[0]+'/rcc')
model_path = dir_names[0]+'/mask_rcnn_nuclei_0003.h5'
"""
model_path = '../input/mask-rcnn-coco-4-script-train/mask_rcnn_nuclei_0003.h5'
#'../input/mask-rcnn-coco-3-script-train/mask_rcnn_nuclei_0003.h5'
#'../input/fork-of-mask-rcnn-coco-2-script-train/mask_rcnn_nuclei_0003.h5'
#'../input/mask-rcnn-coco-1-script/mask_rcnn_nuclei_0003.h5'
#print(dir_names[0]+'/rcc')
#'nuclei20180409T1425/mask_rcnn_nuclei_0003.h5'
#model_path = '../input/mask-rcnn-coco-1/mask_rcnn_nuclei_1.h5'
#model_path = 'mask_rcnn_nuclei_1_30.h5'
#model.find_last()[1]
...
for n, id_ in tqdm(enumerate(test_ids), total=len(test_ids)):
    ...
    ff = 0
    if img.ndim ==2:
        #rle=[[1,img.shape[0]*img.shape[1]]]
        #rles.extend(rle)
        print(n,img.shape,id_)
    ...
    #print(len(results))
        r = results[0]
    #enumerate(['fa751ff3a6332c95cb5cb1d28563553914295e9e7d35c4b6bd267241e8a0787c']):
    #enumerate(test_ids):
    #print(r['masks'].shape[2])
        if r['masks'].shape[2]==0:
            ...
            print(n,len(rle),id_)
            new_test_ids.extend([id_] * len(rle))
        #print ('1',len( rles),len(new_test_ids))
        else:
            mask = np.zeros((img.shape[0], img.shape[1], 1), dtype=np.bool)
            ff = 0
            for i in range(r['masks'].shape[2]):
                mask_ = np.expand_dims(r['masks'][:,:,i], axis=-1)
                if mask_.shape == mask.shape:
                    mask = np.maximum(mask, mask_)
                    ff = 1
            #else:
                #print ('n=',n,'i=',i,pemask_.shape, mask.sha)
        
        #m_b = mask > threshold_otsu(mask) 
        #mask = drop_artifacts(mask, m_b,min_coverage=0.25)
        
        #amask = np.where(mask*255. > 128., 255., 0.).astype(np.uint8)
        #contour = get_contour(amask)
        
        #mask = clean_mask_v2(mask,contour)
        if ff==1 :
            ...
            print(n,len(rle),id_)
            new_test_ids.extend([id_] * len(rle))
            #print (len( rles),len(new_test_ids))
#print (len( rles),len(new_test_ids))
#print(rle)
...
sub.to_csv('sub-dsbowl2018-rcnn-coco-4_0003_fin.csv', index=False)

print(os.listdir("../working"))    
#os.rename('../working/'+dir_names[0]+'/mask_rcnn_nuclei_0003.h5','../working/mask_rcnn_nuclei_0003.h5')
#os.rename('../working/'+dir_names[0]+'/mask_rcnn_nuclei_0002.h5','../working/mask_rcnn_nuclei_0002.h5')
'''
model_path = '../input/fork-of-mask-rcnn-coco-2-script-train/mask_rcnn_nuclei_0002.h5'
print("Loading weights from ", model_path)
model.load_weights(model_path, by_name=True)
new_test_ids = []
rles = []
    ...
    if r['masks'].shape[2]==0:
        ...
        for i in range(r['masks'].shape[2]):
            ...
            if mask_.shape == mask.shape:
                ...
                print ('n=',n,'i=',i,mask_.shape, mask.shape)
...
sub['ImageId'] = new_test_ids
sub['EncodedPixels'] = pd.Series(rles).apply(lambda x: ' '.join(str(y) for y in x))
sub.to_csv('sub-dsbowl2018-rcnn-coco-2_0002.csv', index=False)
'''
